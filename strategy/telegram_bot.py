...
class TelegramBot:
    def __init__(self, rsi_monitor=None):
        ...
        if not self.token or not self.chat_id:
            logging.error("TELEGRAM_TOKEN 또는 TELEGRAM_CHAT_ID가 .env 파일에 설정되지 않았습니다.")
            # base_url을 None으로 설정하여 다른 메서드에서 오류를 발생시키도록 함
            self.base_url = None 
            return
    ...
    def _test_connection(self):
        """
        텔레그램 봇 연결을 테스트하고 시작 메시지를 전송합니다.
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if bot_info.get('ok'):
                    logging.info(f"성공적으로 봇에 연결되었습니다: {bot_info['result']['username']}")
                    # 테스트 메시지 직접 전송
                    msg_url = f"{self.base_url}/sendMessage"
                    data = {"chat_id": self.chat_id, "text": "🤖 RSI 모니터링 봇이 시작되었습니다.", "parse_mode": "HTML"}
                    requests.post(msg_url, data=data, timeout=10)
                else:
                    logging.error(f"봇 정보를 가져오는 데 실패했습니다: {bot_info.get('description')}")
            else:
                logging.error(f"봇 연결에 실패했습니다 (상태 코드: {response.status_code})")
        except requests.exceptions.RequestException as e:
            logging.error(f"봇 연결 테스트 중 오류 발생: {e}")
    ...
    def send_message(self, message):
        ...
        """
        if not self.base_url:
            logging.error("TelegramBot이 제대로 설정되지 않아 메시지를 보낼 수 없습니다.")
            return None
        ...
        try:
            ...
            if response.status_code != 200:
                logging.error(f"메시지 전송 오류: {response.text}")
            return response.json()
        except requests.exceptions.RequestException as e:
            logging.error(f"텔레그램 메시지 전송 중 네트워크 오류: {e}")
            return None
